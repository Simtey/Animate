...
for (var i = firstSelLay; i < lastSelLay + 1; i++) { // loop on each
selected layer
     if (tl.layers[i].layerType !== 'folder') {
         ...
         if (lastSelFrame == stFrame + 1 || lastSelFrame ===
tl.frameCount) { // if the last selected frame is a Key
             tl.setSelectedFrames(firstSelFrame, lastSelFrame);
         } else {
             tl.setSelectedFrames(firstSelFrame, lastSelFrame + 1);
         }
         tl.setSelectedFrames(firstSelFrame, lastSelFrame);
         tl.setFrameProperty('tweenType', 'none'); // delete the motion
tweens
         for (var k = firstSelFrame + 1; k < lastSelFrame; k = k + 2) {
             tl.setSelectedFrames(k, k);
             tl.clearKeyframes(); // delete one frame on 2
             doc.selectAll();
             doc.setElementProperty('loop', 'single frame'); // set the
element on single frame
         }
